...
class EmailService:
    ...
    def send_email(self, from_email, to_list, cc_list, bcc_list, subject, text):
        ...
        for retry in range(len(self.senderservice)):
            status, message = self.senderservice[EmailService.sender_id].send(from_email, to_list, cc_list, bcc_list, subject, text)
            if status == 0:
                message = 'success'
                break
            EmailService.sender_id = (EmailService.sender_id + 1) % len(self.senderservice)
        else:
            status = 4
